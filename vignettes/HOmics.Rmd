...
knitr::opts_chunk$set(cache = TRUE, width = 120,
                      fig.align = "left", out.width="98%", 
                      warning = FALSE,
...
and finally compare results, by selecting those significative by each method. Notice that objects are manipulated differently, as they are of different classes

```{r compare results}
res.homics <- mod$results[[1]]
res.homics.sig <- res.homics %>% filter(`97.5%` < 1 & `2.5%` < 1, n.eff>200) %>% mutate(int.l =`97.5%`- `2.5%`) 
res.homics.sig
...
res.snpassoc.sig$int.l <- res.snpassoc.sig$upper - res.snpassoc.sig$lower
...
```{r agg.matrix, out.width = "30%", echo = FALSE}
knitr::include_graphics("agg.matrix.png")
```
