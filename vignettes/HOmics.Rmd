...
Then the Z matrix providing a priori biological knowledge can be created using this code. Notice that: 1) some SNPs may not be annotated; and 2) a given SNP may have 1 or more than one positions. This is addressed by adding Z matrix by the rownames. 
...
The results are again provided as a HOmics object, and results show this time the coefficients for the multivariate model
...
```{r meth_plot_res, fig.height=3, fig.width=3, out.width="100%", fig.align="center"}
...
Gene expression can be modulated by the miRNAs that bind to genes during transcription. We will see how these binding can affect the association with phenotype.
